...
class Graph
  ...
  def import_graph
    File.readlines('aocdata_sample.txt').each do |line|
  ...
  def valid_neighbor?(let, neighbor)
    validity = false
    validity = true if let == 'E'
    validity = true if neighbor == 'E' && (let == 'z' || let == 'y')
    validity = true if let == 'S' && (neighbor == 'a' || neighbor == 'b')
    validity = true if neighbor == 'S'
    let_ord = let.ord
    if neighbor == 'E' || neighbor == 'S'
      neighbor_ord = 1000
    else
      neighbor_ord = neighbor.ord
    end
    validity = true if neighbor_ord <= let_ord + 1
    validity
  end
